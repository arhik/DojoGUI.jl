...
function AtlasIKerror(mechanism::Mechanism, p_base, p_foot, θ; leg::Symbol = :r)
	...
	set_minimal_coordinates!(mechanism, get_joint(mechanism, Symbol(leg, :_leg_hpx)), [0.0, -θ[1], 0.0])
	set_minimal_coordinates!(mechanism, get_joint(mechanism, Symbol(leg, :_leg_kny)), [θ[2]])
	set_minimal_coordinates!(mechanism, get_joint(mechanism, Symbol(leg, :_leg_akx)), [θ[1]-θ[2], 0.0])
...
function atlas_trajectory(mechanism::Mechanism{T}; timestep=0.05, β=0.5,
	...
	for i = 1:2N
		...
		set_minimal_coordinates!(mechanism, get_joint(mechanism, :back_bkx), [0.0, -αtorso, 0.0])
		set_minimal_coordinates!(mechanism, get_joint(mechanism, :l_leg_hpx), [0.0, -θL[i][1], 0.0])
		set_minimal_coordinates!(mechanism, get_joint(mechanism, :l_leg_kny), [θL[i][2]])
		set_minimal_coordinates!(mechanism, get_joint(mechanism, :l_leg_akx), [θL[i][1]-θL[i][2], 0.0])

		set_minimal_coordinates!(mechanism, get_joint(mechanism, :r_leg_hpx), [0.0, -θR[i][1], 0.0])
		set_minimal_coordinates!(mechanism, get_joint(mechanism, :r_leg_kny), [θR[i][2]])
		set_minimal_coordinates!(mechanism, get_joint(mechanism, :r_leg_akx), [θR[i][1]-θR[i][2], 0.0])

		set_minimal_velocities!(mechanism, get_joint(mechanism, :floating_base), [zeros(3); zeros(3)])
		set_minimal_velocities!(mechanism, get_joint(mechanism, :l_leg_hpx), [0.0, -ωL[i][1], 0.0])
		set_minimal_velocities!(mechanism, get_joint(mechanism, :l_leg_kny), [ωL[i][2]])
		set_minimal_velocities!(mechanism, get_joint(mechanism, :l_leg_akx), [ωL[i][1]-ωL[i][2], 0.0])

		set_minimal_velocities!(mechanism, get_joint(mechanism, :r_leg_hpx), [0.0, -ωR[i][1], 0.0])
		set_minimal_velocities!(mechanism, get_joint(mechanism, :r_leg_kny), [ωR[i][2]])
		set_minimal_velocities!(mechanism, get_joint(mechanism, :r_leg_akx), [ωR[i][1]-ωR[i][2], 0.0])
...
xref = atlas_trajectory(mechanism; 
	timestep=0.05, 
	r=0.0, 
	x=0.03, 
	z=0.85, 
